...
func (g *Generator) generateFunction(fun *repo.Function) {
	...
	if fun.ReturnType == nil {
		...
		} else if fieldType, ok := g.Repo.LookupType(fun.ReturnType.Type); ok {
			switch fieldType.Category {
			case repo.TypeCategoryEnum:
				retMappedType = jen.Id(fieldType.MappedName)
	...
	for _, member := range fun.Members {
		...
		case repo.FieldCategoryDirect:
			...
			memberType, ok := g.Repo.LookupType(member.Type)
			if !ok {
			...
			case repo.TypeCategoryEnum, repo.TypeCategoryBitmask, repo.TypeCategoryHandleNonDispatchable:
				paramsBlock = append(paramsBlock, jen.Id(safeName).Id(memberType.MappedName))
				callBlock = append(callBlock, jen.Qual("C", member.Type).Call(jen.Id(safeName)))
		...
		case repo.FieldCategoryPointer:
			...
			case repo.TypeCategoryEnum, repo.TypeCategoryHandle, repo.TypeCategoryHandleNonDispatchable, repo.TypeCategoryBitmask:
				paramsBlock = append(paramsBlock, jen.Id(safeName).Qual(ffiPath, "Ref").Types(jen.Id(memberType.MappedName)))
